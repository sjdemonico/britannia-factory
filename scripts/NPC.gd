...
func _load_npc_data() -> void:
	...
	talk_sound = str(data.get("talk_sound", "")) if data.has("talk_sound") and data.get("talk_sound") is String else ""
	death_sound = str(data.get("death_sound", "")) if data.has("death_sound") and data.get("death_sound") is String else ""
	var raw_shop_id = data.get("shop_id")
	shop_id = raw_shop_id if raw_shop_id is String else ""
	var npc_defaults: Dictionary = Constants.load_json(Constants.NPC_DEFAULTS_PATH)
	healer_service = bool(data.get("healer_service", false))
	heal_all_price = int(data.get("heal_all_price", npc_defaults.get("heal_all_price", 50)))
	cure_all_price = int(data.get("cure_all_price", npc_defaults.get("cure_all_price", 30)))
	resurrect_price = int(data.get("resurrect_price", npc_defaults.get("resurrect_price", 200)))
