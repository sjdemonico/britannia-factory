...
### Added (Codebase Audit #3)

- **`WorldState.set_flag(key, value)` / `get_flag(key) -> bool` / `has_flag(key) -> bool`** (`autoloads/WorldState.gd`) — three public accessor methods for `WorldState.flags`; `get_flag` returns `false` for missing keys; all callers inside `DialogueManager` updated to use these instead of direct dictionary access
- **`SpellManager.clear_known_spells()`** (`autoloads/SpellManager.gd`) — public method wrapping `_known_spells.clear()`; replaces the private-field access previously used in `SaveManager._reset_all_state()`
- **`StatBlock.get_serializable_modifiers() -> Array`** (`scripts/StatBlock.gd`) — returns modifier instances whose registry entry has `is_status_effect: true`; each entry carries `stat_id`, `modifier_id`, `source_tag`, `ticks_remaining`, `ticks_since_last_application`, and `lifetime_remaining`; equipment and transient light modifiers excluded because they are reapplied from inventory and recalculated at runtime
- **`StatBlock.restore_modifiers_raw(data: Array)`** (`scripts/StatBlock.gd`) — restores serialized status modifiers directly into `_modifiers`; looks up each `modifier_id` in `_modifier_registry` to reconstruct `magnitude`, `stacking`, `duration_type`, and `interval`; skips entries whose `modifier_id` is not in the registry; does not emit stat signals
- **`GameManager.is_any_panel_open() -> bool`** (`autoloads/GameManager.gd`) — formerly `_is_any_panel_open()` (private); all seven call sites in `Player.gd` and `GameManager.gd` updated
- **`GameManager.get_objects_at_tile(tile: Vector2i) -> Array`** (`autoloads/GameManager.gd`) — returns a duplicate of the spatial index entry for the given tile; backed by the new `_object_spatial: Dictionary`
- **`GameManager._object_spatial: Dictionary`** (`autoloads/GameManager.gd`) — spatial index from `Vector2i` → `Array[WorldObject]`; maintained by `register_object_instance` and `unregister_object_instance`; cleared in `_snapshot_and_unload` and `_clear_region`
- **`GameTime.schedule()` tag parameter** (`autoloads/GameTime.gd`) — new optional `tag: String = ""` parameter stored in each scheduled entry; companion `cancel_by_tag(tag: String)` method cancels all matching entries in one call
- **`ui_color_*` variables** (`autoloads/GameManager.gd`) — five designer-configurable colour vars (`ui_color_cursor`, `ui_color_disabled`, `ui_color_text_normal`, `ui_color_text_selected`, `ui_color_text_dim`) loaded from a `"ui_colors"` block in `game.json` via `_parse_color_array()`
- **`assets/audio/test/test_tone.ogg`** — test tone checked into the project under `res://`; replaces the hardcoded filesystem path in `SoundManagerTest._TEST_AUDIO_PATH`; enables SoundManagerTest tests 9–10 and WorldSoundTest test 16 on all machines
- **`data/config/healer_service_types.json`** — data-driven healer service definitions; each entry specifies `id`, `label` template (with `{name}` placeholder for per-member rows), `price_field` (property name on `HealerService`), and `per_downed_member: bool`; `HealerPanel._build_rows()` iterates this file instead of hardcoding the three service types
- **`StatBlockTest`** (`scripts/debug/StatBlockTest.gd`) — 12-test static suite: `load_from_file`, `get_value`, `set_stat`, `modify_stat`, `get_effective_value`, `is_derived`, `get_all_stats`, `get_serializable_modifiers`, `restore_modifiers_raw` empty and with unknown id, `has_stat`
- **`WorldStateTest`** (`scripts/debug/WorldStateTest.gd`) — 12-test suite: `set_flag`/`get_flag`/`has_flag` round-trips; `flags.clear()`; `set_occupant`/`get_occupant`/`is_tile_occupied`/`clear_occupant` lifecycle; `mark_object_tile`/`get_object_ids_at`/`clear_object_from_tile` lifecycle; saves and restores live `WorldState` before and after
- **`FactionManagerTest`** (`scripts/debug/FactionManagerTest.gd`) — 12-test suite: bootstraps `FactionManager.load_from_file` (autoload timing); `FACTIONS_CONFIG_PATH`; faction count; `get_standing`; `set_standing`; clamping to `_scale_max`/`_scale_min`; `modify_standing`; `get_tier`; `get_serializable_standings`; `restore_standings`; unknown faction ignored; `get_faction_name`; restores original standing on exit
- **`SpellManagerTest`** (`scripts/debug/SpellManagerTest.gd`) — 11-test suite: bootstraps `SpellManager.load_registry` (autoload timing); `SPELLS_CONFIG_PATH`; `has_spell` true/false; `get_spell` required fields; `get_casting_stat`; `know_spell`; duplicate `know_spell` returns false; `get_known_spells`; `clear_known_spells`; `can_cast` nonexistent returns false
- **`InventoryTest`** (`scripts/debug/InventoryTest.gd`) — 10-test suite: empty new `Inventory`; `add_object('torch')` returns valid id; `get_objects` size; `add_object` returns -1 when full; `remove_object` by id; `remove_object` missing returns false; two-item add; `restore_objects`; `get_equipped_items` empty; `MAX_SLOTS` positive
- **`RegionDiffTest`** (`scripts/debug/RegionDiffTest.gd`) — 8-test suite: new `RegionDiff` has empty fields; `to_dict` has all required keys; `from_dict` round-trip for `region_id`, `added`, `removed`, `modified`; `from_dict({})` gives empty `region_id`; `to_dict` produces deep copy
- **`QuestManagerTest`** (`scripts/debug/QuestManagerTest.gd`) — 10-test suite: bootstraps `QuestManager.load_registry` (autoload timing); `QUESTS_DATA_PATH`; quest found in registry; `get_quest` known/unknown; `is_quest_active`/`complete`/`failed` unknown → false; `restore_from_state({})` no error; `get_serializable_state` returns Dictionary; round-trip preserves keys
- **`SaveManagerTest`** (`scripts/debug/SaveManagerTest.gd`) — 12-test suite: `SAVES_DIR`; `SAVE_VERSION`; `get_save_index` returns Array; `_object_differs_from_baseline`: identical → false; different tile, `is_open`, `stack_count`, `charges`, `is_lit`, `duration_remaining`, `is_locked`, container contents → all true

### Changed (Codebase Audit #3)

- **`GameTime._load_config()`** (`autoloads/GameTime.gd`) — raw `FileAccess` / `JSON.new()` block replaced with `Constants.load_json(Constants.TIME_CONFIG_PATH)` guarded by `data.is_empty()`
- **`GameTime._fire_scheduled()`** — `Callable.is_valid()` guard added before `cb.call()` to silently skip callbacks from freed nodes instead of erroring
- **`NPC._load_npc_data()`** (`scripts/NPC.gd`) — healer price member variable defaults zeroed at class level; `npc_defaults.json` loaded via `Constants.load_json` in `_load_npc_data()` and used as the fallback source for each healer price field
- **`SoundManager.on_player_tile_changed()`** (`autoloads/SoundManager.gd`) — ambient delay redesigned from a `_process`-based float timer to a `SceneTreeTimer` + sequence counter; `_ambient_change_delay: float` replaces the old constant; `_ambient_timer_seq: int` increments on each tile change and each `_clear_ambient()` call; timer callbacks capture the sequence at creation and no-op if it has advanced; removes the `_process` override and `_update_ambient` method
- **`SoundManager._clear_ambient()`** — `player.queue_free()` added after `player.stop()` in the cleanup loop
- **`SoundManager._crossfade_music()` tween lambda** — `_music_tween = null` added inside the `finished` callback so completed tweens are not treated as still running on the next crossfade request
- **`SoundManager._load_preferences()`** — `push_warning` added on JSON parse failure; previously returned silently with no diagnostic
- **`WorldObject._initialize_sprite(data)`** (`scripts/WorldObject.gd`) — fallback sprite `assert` replaced with `SpriteLoader.load_sprite()` + `push_warning()`; missing corpse or carriable placeholder textures no longer crash debug builds
- **`DialogueManager`** (`scripts/DialogueManager.gd`) — four hardcoded fallback strings replaced with `MessageRegistry.get_message()` calls; direct `WorldState.flags` access replaced with `WorldState.set_flag()` / `get_flag()` calls
- **`DialogueBox`** (`scripts/DialogueBox.gd`) — `SoundManager.play_event("ui_panel_open/close")` added to open and close paths; one hardcoded string replaced with `MessageRegistry.get_message("dialogue_no_dialogue")`; `FileAccess` block in `_load_dialogue_for_member` replaced with `Constants.load_json()`; Escape handler replaced inline close logic with `close()`
- **`GameManager._load_config()`** (`autoloads/GameManager.gd`) — raw `FileAccess` / `JSON.new()` block replaced with `Constants.load_json(Constants.GAME_CONFIG_PATH)`
- **`GameManager.register_object_instance()` / `unregister_object_instance()`** — both maintain `_object_spatial` alongside `_object_instances`
- **`CharacterPanel.close()`** (`scripts/CharacterPanel.gd`) — renamed from `_close()` to `close()`; all callers updated
- **`CharacterPanel._refresh_faction_visible_rows()`** — `const ROW_HEIGHT = 20.0` replaced with a dynamic height derived from `GameManager.get_font_size(Constants.FONT_BODY_ROLE) + 4`
- **`CharacterPanel._build_faction_section()`** — scroll indicators (`▲` / `▼`) inserted above and below the visible item range when the list overflows
- **`HealerPanel._build_rows()`** (`scripts/HealerPanel.gd`) — replaced hardcoded three-row layout with a data-driven loop over `data/config/healer_service_types.json`; `per_downed_member: true` entries generate one row per downed party member with `{name}` substitution; price field name read dynamically from the JSON entry
- **`HealerPanel` item list** — `ScrollContainer` with `SCROLL_MODE_AUTO` wraps the item list `VBoxContainer`, preventing panel overflow when many resurrect rows appear
- **`InventoryScreen._navigate()` / `_navigate_dest()`** (`scripts/InventoryScreen.gd`) — `SoundManager.play_event("ui_button_click")` added after cursor update in both navigation methods
- **`Player.gd`** (`scripts/Player.gd`) — local `_is_any_panel_open()` removed; all three call sites use `GameManager.is_any_panel_open()`; three hardcoded direction-prompt strings replaced with `MessageRegistry.get_message()` calls; `attempt_enter()` posts `MessageRegistry.get_message("enter_nothing")` when target tile is empty
- **`SaveManager._reset_all_state()`** (`autoloads/SaveManager.gd`) — `SpellManager._known_spells.clear()` replaced with `SpellManager.clear_known_spells()`; `WorldState.flags.clear()` and `GameManager._pending_npc_spawns.clear()` added
- **`SaveManager._serialize_party_member()`** — `"modifiers"` key added via `StatBlock.get_serializable_modifiers()`; non-player members additionally serialize `"source_npc_id"`, `"spawn_tile"`, and `"spawn_region_id"`
- **`SaveManager._restore_player_member()`** — `PlayerStats.stat_block.restore_modifiers_raw()` called after `_deserialize_known_spells()`
- **`SaveManager._restore_npc_member()`** — `source_npc_id`, `spawn_tile`, and `spawn_region_id` restored from save data; `stat_block.restore_modifiers_raw()` called before `PartyManager.add_member()`
- **`SaveManager._object_differs_from_baseline()`** — three new checks: `charges` value differs, `is_lit` is `true`, `duration_remaining` differs from baseline
- **`AmbientAudioTest` tests 11–13** (`scripts/debug/AmbientAudioTest.gd`) — updated for the `_ambient_timer_seq` design: dead `_ambient_change_timer` assignments removed; checks replaced with sequence counter comparisons; redundant `queue_free` removed (now handled by `_clear_ambient`)
- **`SoundManagerTest._TEST_AUDIO_PATH`** (`scripts/debug/SoundManagerTest.gd`) — changed from `"F:/Assets/SFX/Mreh.ogg"` to `"res://assets/audio/test/test_tone.ogg"`; `orig_master` → `_orig_master` to suppress unused-variable warning
- **`WorldSoundTest`** — test 16 added: injects a `_test_event` entry into `_sound_registry`, calls `play_event`, confirms a child node was created, cleans up
- **`GameManager._ready()` debug block** — eight new test suites wired: `StatBlockTest`, `WorldStateTest`, `FactionManagerTest`, `SpellManagerTest`, `InventoryTest`, `RegionDiffTest`, `QuestManagerTest`, `SaveManagerTest`

### Fixed (Codebase Audit #3)

- **Ambient player nodes leaked on every region transition** (`autoloads/SoundManager.gd`) — `_clear_ambient()` stopped ambient `AudioStreamPlayer` nodes but never called `queue_free()`; each region transition accumulated unreleased nodes
- **Completed music tween treated as active on the next crossfade** (`autoloads/SoundManager.gd`) — `_music_tween` was never nulled after its `finished` callback; `_crossfade_music()` could call `.is_running()` or `.kill()` on a dead tween; fixed by setting `_music_tween = null` inside the `finished` lambda
- **NPC healer price defaults ignored `npc_defaults.json`** (`scripts/NPC.gd`) — both the class-level default and the `data.get()` fallback for all three healer price fields were hardcoded integers; designer changes to `npc_defaults.json` had no effect
- **Missing sprite textures crashed debug builds** (`scripts/WorldObject.gd`) — `assert(SpriteLoader.load_sprite(...))` in the fallback sprite chain crashed when corpse or carriable placeholder textures were missing; replaced with `push_warning()` and graceful null-texture fallback
- **`SaveManager._reset_all_state()` did not clear `WorldState.flags` or pending NPC spawns** (`autoloads/SaveManager.gd`) — flags from a prior session could affect dialogue conditions and quest triggers in a new game; pending NPC spawns could duplicate NPC placement on the first region load after new-game
- **`SaveManager` accessed `SpellManager._known_spells` directly** — bypassed the property getter that routes through `player.known_spells` when an active player member exists; replaced with `SpellManager.clear_known_spells()`
- **NPC party member spawn data not serialized** (`autoloads/SaveManager.gd`) — `source_npc_id`, `spawn_tile`, and `spawn_region_id` were omitted from `_serialize_party_member()` for recruited NPC members; on reload, NPC members were created with empty spawn data, preventing correct world placement on dismiss
- **Status-effect modifiers not persisted across save/load** — active timed modifiers (spell buffs, debuffs) on party member stat blocks were discarded on save; reloading restored only base stat values
- **`_object_differs_from_baseline()` missed charges, lit state, and duration** (`autoloads/SaveManager.gd`) — partially consumed items, lit torches, and items with reduced duration would not generate region diff entries; their state would reset to JSON defaults on reload

### Changed (Sprite sizing enforcement)

- **`SpriteLoader.apply_to_node()` for `Sprite2D`** (`scripts/SpriteLoader.gd`) — scale computation changed from `Vector2(size) / Vector2(Constants.SPRITE_SOURCE_SIZE)` to `Vector2(size) / Vector2(texture.get_width(), texture.get_height())`; output size is now correct for any source texture regardless of its pixel dimensions; `push_warning` emitted if actual dimensions differ from `SPRITE_SOURCE_SIZE`
- **`WorldObject._initialize_sprite()` fallback paths** (`scripts/WorldObject.gd`) — corpse, carriable, and noncarriable placeholder sprite assignments replaced with `SpriteLoader.apply_to_node(sprite, path, Constants.SPRITE_WORLD_SIZE)`; scale is now set correctly for all three fallback cases instead of being left at whatever value the node inherited

### Removed

- **All 16 debug test suites** (`scripts/debug/`) — `SpriteInfraTest`, `AnimationTest`, `UIArtTest`, `SoundManagerTest`, `WorldSoundTest`, `CombatSoundTest`, `UISoundTest`, `AmbientAudioTest`, `StatBlockTest`, `WorldStateTest`, `FactionManagerTest`, `SpellManagerTest`, `InventoryTest`, `RegionDiffTest`, `QuestManagerTest`, `SaveManagerTest` — all tests passed; suites deleted after verification; `GameManager._ready()` debug block removed

---

## [Unreleased] — 2026-07-18

### Added (M24a — SoundManager and Audio Infrastructure)

- **`SoundManager`** (`autoloads/SoundManager.gd`) — new autoload registered after `FactionManager`; owns all audio playback; initialises on startup by loading `game.json` audio defaults, applying any designer-supplied bus layout, calling `_ensure_buses()`, loading user preferences, applying bus volumes, then creating three persistent `AudioStreamPlayer` children (`_music_player_a`, `_music_player_b` on the Music bus; `_ambient_player` on the Ambient bus)
- **`SoundManager.play_sfx(path)`** — creates a new `AudioStreamPlayer` on the SFX bus per call; connects `finished` to `queue_free` so players self-destruct; unlimited simultaneous channels; empty or missing path returns silently with no player created
- **`SoundManager.play_music(path)`** — crossfades between tracks over `music_crossfade_duration` seconds using a `Tween` (`set_parallel(true)`, fade active player to −80 dB, fade inactive player from −80 dB to 0 dB); calling with the same path already playing returns immediately without restart; calling with an empty path stops all music; an in-progress crossfade is killed and both players reset to a known state before the new fade begins
- **`SoundManager.play_ambient(path)`** — plays a looping audio stream on the Ambient bus; sets `loop = true` on `AudioStreamOggVorbis` / `AudioStreamMP3` streams and `loop_mode = LOOP_FORWARD` on `AudioStreamWAV`; calling with the same path already playing returns immediately; empty path stops ambient
- **`SoundManager.stop_music()` / `stop_ambient()`** — stop playback and clear the current-path trackers; both guard against null players (safe to call before `_ready()` completes, e.g. from test suites)
- **`SoundManager.set_volume(bus, value)`** — clamps `value` to 0.0–1.0, stores it in the matching `_volume_*` var, converts to dB via `_to_db()` (maps ≤ 0.001 linear to −80 dB to avoid `-INF`), applies to the audio bus via `AudioServer.set_bus_volume_db()`, then writes `user://preferences.json`
- **`SoundManager.get_volume(bus) -> float`** — returns the stored linear volume for the named bus; unknown bus returns 0.0
- **`SoundManager._ensure_buses()`** — called during `_ready()` after any designer bus layout is applied; for each of Music, SFX, Ambient: if `AudioServer.get_bus_index()` returns −1, calls `AudioServer.add_bus()` then reads `get_bus_count() - 1` for the new index (Godot 4 `add_bus()` returns `void`), names it, and sets its send to Master; prevents crashes if the `.tres` bus layout fails to load
- **`SoundManager._load_audio(path) -> AudioStream`** — routes `res://` / `user://` paths through `ResourceLoader.exists()` + `load()`; routes absolute filesystem paths through `FileAccess.get_file_as_bytes()` + `AudioStreamOggVorbis.load_from_buffer()` (OGG) or `AudioStreamMP3.new(); stream.data = bytes` (MP3); returns `null` silently for empty, missing, or unsupported paths
- **`SoundManager._save_preferences()` / `_load_preferences()`** — `_save_preferences()` writes all four volumes to `user://preferences.json` as `{"volume": {"master":…, "music":…, "sfx":…, "ambient":…}}`; called by `set_volume()` on every change; `_load_preferences()` reads the file on startup, falls back per-key to `_audio_defaults` (the game.json defaults stored during `_ready()`) for any missing key, and clamps all loaded values to 0.0–1.0; missing file is a no-op (game.json defaults apply)
- **`audio/default_bus_layout.tres`** — text-format `AudioBusLayout` resource defining four buses: `Master` (root, no send), `Music → Master`, `SFX → Master`, `Ambient → Master`; registered as the project default via `project.godot [audio] buses/default_bus_layout`; Godot loads this before any script runs so buses exist when `SoundManager._ready()` fires; `_ensure_buses()` provides a code-level fallback if the resource fails to load
- **`SoundManagerTest`** (`scripts/debug/SoundManagerTest.gd`) — 18-test static suite wired into `GameManager._ready()` after `UIArtTest.run_static()`; tests 1–5 verify the five new `Constants` values; test 6 asserts all four buses exist; tests 7–8 assert `play_sfx` with empty/missing path creates no child node; tests 9–10 assert `play_sfx` with a valid file creates a player (and multiple simultaneous calls create independent players) — skipped if `_TEST_AUDIO_PATH` does not exist on disk; tests 11–13 assert empty-path calls to `play_music` and `play_ambient` clear their current-path trackers and stop players without crashing (null-player guard allows these to run before `SoundManager._ready()` completes); tests 14–16 verify `set_volume` stores, clamps high, and clamps low; test 17 verifies `user://preferences.json` is written after `set_volume`; test 18 writes a known preferences file, calls `_load_preferences()`, and asserts all four volumes loaded correctly; test suite restores volumes to game.json defaults on exit
- **`Constants.AUDIO_BUS_MASTER`** (`"Master"`), **`AUDIO_BUS_MUSIC`** (`"Music"`), **`AUDIO_BUS_SFX`** (`"SFX"`), **`AUDIO_BUS_AMBIENT`** (`"Ambient"`), **`PREFERENCES_PATH`** (`"user://preferences.json"`) — five new string constants added to `autoloads/Constants.gd`
- **`"audio"` block** added to `data/config/game.json` — `"bus_layout_path": null` (null uses the project default; set to a `.tres` path to override at runtime), `"volume": {"master": 1.0, "music": 0.8, "sfx": 1.0, "ambient": 0.6}` (linear floats 0.0–1.0; serve as fallback defaults when `user://preferences.json` is absent or a key is missing), `"music_crossfade_duration": 1.0` (seconds)

### Changed (M24a — SoundManager and Audio Infrastructure)

- **`project.godot`** — `SoundManager` autoload added between `FactionManager` and `SaveManager`; `[audio] buses/default_bus_layout` set to `"res://audio/default_bus_layout.tres"` so Godot installs the four-bus layout before the first scene runs
- **`GameManager._ready()` debug block** — `SoundManagerTest.run_static()` added after `UIArtTest.run_static()`

### Added (M24b — World Map Sound Hooks)

- **`data/config/sounds.json`** — global event-sound registry; 12 null-by-default event keys: `item_pickup`, `item_drop`, `door_open`, `container_open`, `trap_triggered`, `combat_start`, `combat_end`, `spell_cast`, `level_up`, `quest_complete`, `attack_hit`, `attack_miss`; designer assigns a file path (absolute or `res://`) to any key; unassigned keys play silence
- **`SoundManager.play_event(event_id)`** (`autoloads/SoundManager.gd`) — looks up `event_id` in `_sound_registry`; plays the path via `play_sfx()` if the value is a non-empty String; empty/missing/null values are silent no-ops; empty `event_id` returns immediately
- **`SoundManager._sound_registry`** / **`_load_sound_registry()`** — `_sound_registry: Dictionary` stores the loaded event map; `_load_sound_registry()` reads `Constants.SOUNDS_CONFIG_PATH` via `Constants.load_json()` and stores the result; called at the end of `SoundManager._ready()`
- **`Constants.SOUNDS_CONFIG_PATH`** (`"res://data/config/sounds.json"`) — new constant in `autoloads/Constants.gd`
- **`footstep_sound` / `hazard_sound`** fields added to all 8 tile entries in `data/config/tiles.json` — both default to `null`; `footstep_sound` plays on every successful player step onto that tile type; `hazard_sound` plays once per tick in which at least one hazard fires on the tile
- **`TileRegistry`** (`scripts/TileRegistry.gd`) — `load_from_file()` now reads and stores `footstep_sound` and `hazard_sound` for each tile; `null` JSON values stored as `""` (empty string); accessible via `get_tile(tile_id)`
- **`GameManager.on_player_moved(tile)`** (`autoloads/GameManager.gd`) — new public method; looks up the tile definition for the given tile position via `tile_registry`; calls `SoundManager.play_sfx()` with `footstep_sound` if non-empty; called by `Player.attempt_move()` immediately after `GameManager.player_tile` is updated
- **`time_periods` format migrated** in `data/config/time.json` — entries changed from flat integers (`"dawn": 5`) to objects (`"dawn": {"hour": 5, "sound": null}`); the `sound` field holds a path for the transitional sound played when that period begins; all four periods (`dawn`, `day`, `dusk`, `night`) default to `null`
- **`GameTime._time_period_sounds`** / **`get_period_sound(period)`** (`autoloads/GameTime.gd`) — new `_time_period_sounds: Dictionary` stores the sound path per period name; populated during `_load_config()` alongside `_time_periods` (which retains its original int-hours structure so `_compute_ambient_radius()` and `_rebuild_sorted_periods()` are unchanged); `get_period_sound(period) -> String` is the public accessor; old flat-int format is handled as a fallback (sound defaults to `""`)
- **`"sound_path": null`** added to every `use_actions` entry in `data/objects/objects.json` (34 action entries across 23 objects) — designer-configurable per-action sound played by `GameManager.execute_use()` on each successful action in the chain
- **`"talk_sound": null`** added to all 13 NPC JSON files — designer-configurable path played via `SoundManager.play_sfx()` when a dialogue window opens on that NPC; `null` / empty string plays silence
- **`NPC.talk_sound`** (`scripts/NPC.gd`) — new `var talk_sound: String = ""`; loaded from JSON in `_load_npc_data()` as a String or empty string if the field is absent or not a String
- **`WorldSoundTest`** (`scripts/debug/WorldSoundTest.gd`) — 15-test static suite wired into `GameManager._ready()` after `SoundManagerTest.run_static()`; test 1 asserts `SOUNDS_CONFIG_PATH`; tests 2–4 assert `_sound_registry` has `item_pickup`, `item_drop`, and ≥ 12 keys (bootstraps registry via `_load_sound_registry()` since `SoundManager._ready()` has not yet run at test time); tests 5–7 assert `play_event` with empty string / unknown key / null-path known key creates no audio players; tests 8–12 assert all four period names exist in `GameTime._time_period_sounds`; test 12 asserts `get_period_sound` returns a String; tests 13–14 assert `tile_registry` stores `footstep_sound` and `hazard_sound` fields for `grass` and `swamp`; test 15 asserts `goblin_grunt.json` has the `talk_sound` field

### Changed (M24b — World Map Sound Hooks)

- **`SoundManager._ready()`** — `_load_sound_registry()` call added at the end of init, after the three `AudioStreamPlayer` children are created
- **`GameTime._load_config()`** (`autoloads/GameTime.gd`) — `time_periods` parsing replaced: if a period value is a Dictionary the method reads `"hour"` into `_time_periods` and `"sound"` into `_time_period_sounds`; if the value is a plain int (old format) it is stored directly and the sound defaults to `""`; `_time_periods` values remain plain ints throughout so `_rebuild_sorted_periods()` and `_compute_ambient_radius()` require no changes
- **`GameManager._on_time_period_changed()`** (`autoloads/GameManager.gd`) — now calls `GameTime.get_period_sound(period)` and forwards any non-empty result to `SoundManager.play_sfx()` before dispatching to the period-specific handler
- **`GameManager.execute_use()`** (`autoloads/GameManager.gd`) — per-action sound hook added: after `use_action_registry.execute()` returns `true`, reads `action_entry.get("sound_path", "")` and calls `SoundManager.play_sfx()`; the chain still breaks on `false` as before
- **`HazardProcessor._process_tile_hazards()`** (`scripts/HazardProcessor.gd`) — resolves the tile definition once per call; after the first hazard that actually fires (passes trigger and immunity checks), plays `tile_def.get("hazard_sound", "")` via `SoundManager.play_sfx()` at most once per tick regardless of how many hazards fire on the tile
- **`Player.attempt_move()`** (`scripts/Player.gd`) — `GameManager.on_player_moved(tile_pos)` called immediately after `GameManager.player_tile = tile_pos`
- **`Player._do_get()`** (`scripts/Player.gd`) — `SoundManager.play_event("item_pickup")` called after `MessageLog.post_blank()` on both the single and plural pickup paths
- **`Player._resolve_drop()`** (`scripts/Player.gd`) — `SoundManager.play_event("item_drop")` added after `MessageLog.post_blank()` on the container-deposit path and on the ground-drop path
- **`Player._start_dialogue()`** (`scripts/Player.gd`) — `SoundManager.play_sfx(npc.talk_sound)` called before `dialogue_box.open(npc)` if `npc.talk_sound` is non-empty
- **`GameManager._ready()` debug block** — `WorldSoundTest.run_static()` added after `SoundManagerTest.run_static()`

### Added (M24c — Combat Sound Hooks)

- **8 new keys in `data/config/sounds.json`** — `spell_cast_default`, `weapon_swing_default`, `weapon_hit_default`, `weapon_miss`, `npc_death_default`, `player_death`, `arena_entry`, `arena_exit`; all null by default; designer assigns paths to activate each hook
- **`SoundManager.get_registry(event_id) -> String`** (`autoloads/SoundManager.gd`) — looks up `event_id` in `_sound_registry`; returns the path String if the value is a non-empty String, otherwise returns `""`; used by combat code to build per-weapon / per-spell → global fallback chains without duplicating lookup logic
- **`"cast_sound": null`** added to all 6 spell entries in `data/config/spells.json` — per-spell cast sound; if non-null it takes priority over `spell_cast_default`; null falls through to the global fallback
- **`"swing_sound": null` / `"hit_sound": null`** added to `data/config/object_defaults.json` and to all 3 weapon object entries in `data/objects/objects.json` (`sword_iron`, `sword_twohanded`, `bow_short`) — per-weapon sounds; null values fall back to `weapon_swing_default` / `weapon_hit_default` from the registry; non-weapon objects inherit the null defaults and are never queried in combat
- **`"death_sound": null`** added to all 13 NPC JSON files — per-NPC death sound; null falls back to `npc_death_default` from the registry
- **`NPC.death_sound`** (`scripts/NPC.gd`) — new `var death_sound: String = ""`; loaded from JSON in `_load_npc_data()` as a String or `""` if the field is absent or not a String
- **`CombatSoundTest`** (`scripts/debug/CombatSoundTest.gd`) — 15-test static suite; bootstraps `SoundManager._load_sound_registry()` and `SpellManager.load_registry()` at the start (both autoloads initialise after GameManager in the project autoload order); tests 1–2 verify `get_registry` return type and graceful unknown-key handling; tests 3–10 verify all 8 new registry keys are present in `_sound_registry`; test 11 verifies spell `heal` has `cast_sound` field; test 12 verifies `goblin_grunt.json` has `death_sound` field; tests 13–14 verify `object_defaults.json` has `swing_sound` and `hit_sound`; test 15 verifies `play_event("weapon_miss")` with null path creates no audio player

### Changed (M24c — Combat Sound Hooks)

- **`CombatManager.resolve_attack()`** (`autoloads/CombatManager.gd`) — three sound calls added: `_play_swing_sound(attacker)` after `_consume_ammo()` and before hit/miss resolution (plays on every attack attempt, ranged or melee); `SoundManager.play_event("weapon_miss")` on the miss branch before returning; `_play_hit_sound(attacker)` on the hit branch before damage calculation
- **`CombatManager._play_swing_sound()` / `_play_hit_sound()`** (`autoloads/CombatManager.gd`) — new private helpers; each reads the corresponding sound field from the attacker's equipped weapon data (`weapon.get("data", {}).get("swing_sound/hit_sound", null)`); falls back to `SoundManager.get_registry("weapon_swing/hit_default")` if the per-weapon field is empty or null; delegates to `SoundManager.play_sfx()`; no sound plays if both are null/empty
- **`CombatManager._handle_death()`** (`autoloads/CombatManager.gd`) — NPC death path: reads `death_sound` from the combatant's NPC node; falls back to `SoundManager.get_registry("npc_death_default")`; plays via `SoundManager.play_sfx()` before the slain message and corpse spawn
- **`CombatManager.show_mortis()`** (`autoloads/CombatManager.gd`) — `SoundManager.play_event("player_death")` called immediately after `in_combat = false`, before the MortisScreen scene is loaded and instantiated
- **`CombatManager.end_combat()`** (`autoloads/CombatManager.gd`) — `SoundManager.play_event("arena_exit")` called immediately after `in_combat = false`, before `_player_turn_finished` is emitted
- **`CombatArena.initialize()`** (`scripts/CombatArena.gd`) — `SoundManager.play_event("arena_entry")` called as the first statement, before arena generation begins
- **`SpellManager.attempt_cast()`** (`autoloads/SpellManager.gd`) — after `consume_cast_resources()`, reads `spell.get("cast_sound", null)`; if non-empty String plays it directly; otherwise calls `SoundManager.get_registry("spell_cast_default")` as fallback; delegates to `SoundManager.play_sfx()`; plays before the spell-cast message log entry
- **`GameManager._ready()` debug block** — `CombatSoundTest.run_static()` added after `WorldSoundTest.run_static()`

### Added (M24d — UI Sound Hooks)

- **6 new keys in `data/config/sounds.json`** — `ui_panel_open`, `ui_panel_close`, `ui_button_click`, `ui_confirm`, `ui_error`, `ui_scroll`; all null by default; designer assigns paths to activate each hook
- **`UISoundTest`** (`scripts/debug/UISoundTest.gd`) — 12-test static suite; bootstraps `SoundManager._load_sound_registry()`; tests 1–6 verify all 6 new keys are present in `_sound_registry`; tests 7–12 verify `play_event()` on each key with a null path creates no audio player

### Changed (M24d — UI Sound Hooks)

- **`ScrollableList.scroll_up()` / `scroll_down()`** (`scripts/ScrollableList.gd`) — both methods capture `_scroll_offset` before mutation and call `SoundManager.play_event("ui_scroll")` only when the offset actually changed; prevents false firing when already at the top or bottom boundary
- **`CommandIconBar._on_icon_clicked()`** (`scripts/CommandIconBar.gd`) — `SoundManager.play_event("ui_button_click")` added as the first statement
- **`InventoryScreen.open()` / `close()`** (`scripts/InventoryScreen.gd`) — `ui_panel_open` on open; `ui_panel_close` guarded by `panel.visible` check on close (prevents double-firing when `GameManager.open_panel()` calls `close()` on already-hidden panels)
- **`CharacterPanel.open_at_index()` / `_close()`** (`scripts/CharacterPanel.gd`) — `ui_panel_open` inside the `if not panel.visible:` guard; `ui_panel_close` guarded by `panel.visible` in `_close()` (GameManager calls `_close()` not `close()` for this panel)
- **`JournalPanel.open()` / `close()`** (`scripts/JournalPanel.gd`) — `ui_panel_open` on open; `ui_panel_close` guarded by `panel.visible` on close
- **`SpellbookPanel.open()` / `close()`** (`scripts/SpellbookPanel.gd`) — `ui_panel_open` on open; `ui_panel_close` guarded by `panel.visible` on close; `ui_button_click` in `_move_cursor()` after cursor update and in `_on_row_gui_input()` on cursor-change branch (not double-click branch)
- **`ShopPanel.open()` / `close()`** (`scripts/ShopPanel.gd`) — `ui_panel_open` on open; `ui_panel_close` guarded by `panel.visible` on close; `ui_button_click` in `_navigate()`, `_on_arrow_button_down()`, `_on_tab_label_clicked()` (after valid-click guard), and `_on_row_gui_input()` cursor-change branch; `ui_error` in `_enter_quantity_mode()` on can't-afford exit and in `_do_buy()` on each of three failure paths (cannot afford, carry limit, out of stock); `ui_confirm` after `_do_buy()` success and after `_do_sell()` success
- **`HealerPanel.open()` / `close()`** (`scripts/HealerPanel.gd`) — `ui_panel_open` on open; `ui_panel_close` guarded by `panel.visible` on close; `ui_button_click` in `_navigate()` and `_on_row_gui_input()` cursor-change branch; `ui_error` on both failure returns in `_purchase()`; `ui_confirm` after the match block on purchase success
- **`SaveLoadPanel.open()` / `close()`** (`scripts/SaveLoadPanel.gd`) — `ui_panel_open` on open; `ui_panel_close` guarded by `panel.visible` on close; `ui_button_click` in `_on_slot_row_gui_input()` cursor-change branch; `ui_confirm` after each of the four `_execute_confirmed()` outcomes (save-here, overwrite, load, delete)
- **`GameManager._ready()` debug block** — `UISoundTest.run_static()` added after `CombatSoundTest.run_static()`

### Added (M24e — Ambient Audio)

- **`"music_path": null`** added to `data/regions/wilderness.json` and `data/regions/town.json` — designer-configurable per-region background music track; null means no region music
- **`"music_path": null`** added to all 4 time period entries in `data/config/time.json` — designer-configurable per-period music track; played when that period is active and no higher-priority layer is playing
- **`"combat_music": null`** added to `data/config/sounds.json` — designer-configurable combat music track; plays during arena and is looked up via `SoundManager.get_registry("combat_music")`
- **`"ambient_sound": null`** added to all 8 tile type entries in `data/config/tiles.json` — designer-configurable looping ambient sound per tile type; fades in when the player enters that tile type (after a 3-second delay) and fades out when they leave
- **`GameTime._time_period_music`** / **`get_period_music(period) -> String`** (`autoloads/GameTime.gd`) — new `_time_period_music: Dictionary` stores the music path per period name; populated during `_load_config()` alongside `_time_period_sounds` from the `"music_path"` field of each period entry; `get_period_music()` is the public accessor; old flat-int period format defaults music to `""`
- **`TileRegistry` ambient_sound field** (`scripts/TileRegistry.gd`) — `load_from_file()` now reads and stores `ambient_sound` for each tile; null JSON values stored as `""`; accessible via `get_tile(tile_id)`
- **Music priority system** (`autoloads/SoundManager.gd`) — three-layer priority: combat (highest) > time_of_day > region (lowest); tracked in `_music_priority: String` and three path vars (`_combat_music_path`, `_time_music_path`, `_region_music_path`); `_resolve_music()` picks the highest non-empty layer and calls `play_music()`, which handles the crossfade and same-track no-restart logic
- **`SoundManager.set_region_music(path)`** — stores path in `_region_music_path` and calls `_resolve_music()`; accepts null (treated as empty string); called by `GameManager._fresh_load_region()` and `_restore_from_cache()` on every region load
- **`SoundManager.set_time_music(path)`** — stores path in `_time_music_path` and calls `_resolve_music()`; called by `GameManager._on_time_period_changed()` on every period change; also re-applied on region load so initial period music is set correctly for fresh games
- **`SoundManager.set_combat_music(path)`** — stores path in `_combat_music_path` and calls `_resolve_music()`; empty string clears combat layer and immediately resumes the next-highest layer via crossfade
- **Tile ambient loop system** (`autoloads/SoundManager.gd`) — `_ambient_players: Dictionary` (tile_type_id → AudioStreamPlayer) allows multiple tile type ambient loops to coexist simultaneously; `_pending_ambient_tile_type` and `_ambient_change_timer` implement a 3-second delay before switching; `_current_ambient_tile_type` tracks the active tile; `_update_ambient(delta)` runs in `_process()` and fires `_switch_ambient()` when the delay elapses; rapid tile changes reset the timer each time
- **`SoundManager.on_player_tile_changed(tile_type_id)`** — records the new tile type as pending; resets the ambient change timer; called by `GameManager.on_player_moved()` on every player step
- **`SoundManager._switch_ambient(tile_type_id)`** — looks up `ambient_sound` from `GameManager.tile_registry`; crossfades out the old tile type's player (via `_crossfade_out_ambient()`), then starts and crossfades in the new one (via `_crossfade_in_ambient()`); both use Tween over `_crossfade_duration`; if the new tile type has no ambient path, fades out and goes silent; if the player returns to an already-playing tile type's player, it is not restarted
- **`SoundManager._clear_ambient()`** — kills all in-progress ambient tweens, stops all ambient players, and clears `_ambient_players`, `_ambient_tweens`, `_current_ambient_tile_type`, `_pending_ambient_tile_type`, and `_ambient_change_timer`; called by `GameManager` on region unload
- **`AmbientAudioTest`** (`scripts/debug/AmbientAudioTest.gd`) — 14-test static suite wired into `GameManager._ready()` after `UISoundTest.run_static()`; tests 1–5 verify structural data (registry key, tile ambient fields, time period music fields, region JSON music fields); tests 6–10 verify the three-layer music priority (`set_region_music` → priority `"region"`, `set_time_music` → priority `"time_of_day"`, `set_combat_music` → priority `"combat"`, clearing combat → falls back to `"time_of_day"`, all null → priority `"none"`); tests 11–12 verify `on_player_tile_changed` sets the pending tile type and rapid changes reset the delay timer; test 13 verifies `_clear_ambient` resets all ambient state; test 14 verifies `play_music` with the same path is a no-op (continuous playback preserved)

### Changed (M24e — Ambient Audio)

- **`SoundManager._process(delta)`** (`autoloads/SoundManager.gd`) — new `_process()` override added; calls `_update_ambient(delta)` each frame to advance the ambient change delay timer
- **`GameManager._fresh_load_region()`** (`autoloads/GameManager.gd`) — calls `SoundManager.set_region_music(region_data.get("music_path", null))` and `SoundManager.set_time_music(GameTime.get_period_music(GameTime.get_time_period()))` at the end of every fresh region load; the time music re-application ensures the correct period track plays on initial game load (before any `time_period_changed` signal fires)
- **`GameManager._restore_from_cache()`** (`autoloads/GameManager.gd`) — same two `SoundManager` calls added at the end so music is correctly re-applied when returning to a cached region
- **`GameManager._snapshot_and_unload()`** (`autoloads/GameManager.gd`) — `SoundManager._clear_ambient()` called as the first statement; stops and clears all tile ambient loops before the old region is freed
- **`GameManager._clear_region()`** (`autoloads/GameManager.gd`) — `SoundManager._clear_ambient()` called as the first statement; guards the combat arena → world transition path
- **`GameManager.on_player_moved()`** (`autoloads/GameManager.gd`) — `SoundManager.on_player_tile_changed(tile_id)` called after the footstep sound, passing the resolved tile type ID; triggers the ambient delay/switch pipeline
- **`GameManager._on_time_period_changed()`** (`autoloads/GameManager.gd`) — `SoundManager.set_time_music(GameTime.get_period_music(period))` called after the period transition SFX and before the period-specific handler (`_on_dawn`, `_on_day`, etc.)
- **`CombatManager.initiate_combat()`** and **`initiate_combat_with_group()`** (`autoloads/CombatManager.gd`) — `SoundManager.set_combat_music(SoundManager.get_registry("combat_music"))` called before `GameManager.load_region("combat_arena")`; ensures combat layer is active before region music resets
- **`CombatManager.end_combat()`** (`autoloads/CombatManager.gd`) — `SoundManager.set_combat_music("")` called as the first statement; immediately starts crossfade back to time-of-day or region music before the arena exit SFX fires
- **`GameManager._ready()` debug block** — `AmbientAudioTest.run_static()` added after `UISoundTest.run_static()`
